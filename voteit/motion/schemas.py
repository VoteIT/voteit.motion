    ...
    hashlist_uids = colander.SchemaNode(
        ...
        colander.SchemaNode(
            ...
            widget=ReferenceWidget(
                query_params={"type_name": "HashList"}, multiple=False
            ),
        ),
        title=_(
            "hashlist_uids_schema_title",
            default="Users in this hashlist may gain permission to participate",
        ),
        description=_(
            "hashlist_uids_schema_description",
...
class MotionSchema(colander.Schema):
    ...
    proposals = colander.SchemaNode(
        ...
        description=_(
            "motion_proposals_schema_description",
...
class EditEndorsementsSchema(colander.Schema):
    ...
    endorsements_text = colander.SchemaNode(
        ...
        description=_(
            "written_endorsements_description",
...
def _get_meetings(request):
    docids = request.root.catalog.query(
        "type_name == 'Meeting'", sort_index="sortable_title"
    )[1]
    values = []
        ...
        schema.add(
            colander.SchemaNode(
                ...
                validator=colander.Regex(
                    _HASHTAG_PATTERN, msg=_("Must be a-z, 0-9, or any of '.-_'.")
                ),
            )
        )
